...
blk_mdl_all_slides_leaveout_patient = cell(patient_num,1);
blk_train_all_slides_leaveout_patient = cell(patient_num,1);
blk_test_all_slides_leaveout_patient = cell(patient_num,1);

if (PARALLEL_PROCESSING) % don't forget to switch FOR/PARFOR below
    parpool; % matlabpool;
end % PARALLEL_PROCESSING

parfor leaveout_indx = 1:patient_num % <== Change FOR to PARFOR to use parallel processing...
    tic;
    LEAVEOUT_PATIENTS = patient_vec(leaveout_indx); % leaveout_indx;
    % disp(['Starting leaveout patient ',num2str(leaveout_indx),'/',num2str(patient_num)]);
    ...
    % optional logistics...
    % blk_train = cell(slide_num,1);
    % blk_test = cell(slide_num,1);
    % blk_feat_norm = cell(slide_num,1);
    % for slide_indx = 1:slide_num
    %     blk_train{slide_indx} = blk_train_all_slides(blk_slide_all_slides == slide_indx);
    %     blk_test{slide_indx} = blk_test_all_slides(blk_slide_all_slides == slide_indx);
    %     blk_feat_norm{slide_indx} = blk_feat_norm_all_slides(blk_slide_all_slides == slide_indx,:);
    % end % for slide_indx
    % clearvars('blk_feat', 'blk_feat_all_slides', 'blk_feat_norm'); % to save memory...
    ...
    blk_mdl_all_slides_leaveout_patient{leaveout_indx} = blk_mdl_all_slides;
    
    % more optional logistics...
    blk_train_all_slides_leaveout_patient{leaveout_indx} = blk_train_all_slides;
    blk_test_all_slides_leaveout_patient{leaveout_indx} = blk_test_all_slides;
    
    disp(['Finished leaveout patient ',num2str(leaveout_indx),'/',num2str(patient_num),' at ',num2str(toc),'sec']);
...
blk_mdl_leaveout = cell(slide_num,1); % the results of the leaveout slides...
for leaveout_indx = 1:patient_num
    LEAVEOUT_PATIENTS = patient_vec(leaveout_indx); % leaveout_indx;
    % leaveout_slides = unique( blk_slide_all_slides(ismember(blk_patient_all_slides,LEAVEOUT_PATIENTS)) );
    
    for slide_indx = 1:slide_num
        blk_mdl_leaveout{slide_indx}( (ismember(blk_patient{slide_indx},LEAVEOUT_PATIENTS)) & blk_valid{slide_indx}, 1 ) = ...
            blk_mdl_all_slides_leaveout_patient{leaveout_indx}( (blk_slide_all_slides == slide_indx) & ...
            (ismember(blk_patient_all_slides,LEAVEOUT_PATIENTS)) & (blk_valid_all_slides) );
        blk_mdl_leaveout{slide_indx}(~blk_valid{slide_indx},1) = INVALID_CLASS;
    end % slide_indx
end % leaveout_indx
blk_mdl_leaveout_all_slides = cat(1,blk_mdl_leaveout{:});

% block-wise analysis
conf_mat_leaveout = zeros(label_num,label_num,slide_num);
for slide_indx = 1:slide_num
    conf_mat_leaveout(:,:,slide_indx) = confusionmat(blk_label{slide_indx}(blk_valid{slide_indx}),...
        blk_mdl_leaveout{slide_indx}(blk_valid{slide_indx}), 'order', [1:label_num]-1);
end % for slide_indx
percentResultClassify = bsxfun(@rdivide,sum(conf_mat_leaveout,3),sum(sum(conf_mat_leaveout,3)+eps,2));
disp( bsxfun(@rdivide,sum(conf_mat_leaveout,3),sum(sum(conf_mat_leaveout,3)+eps,2)) ); % convert confusion matrix into percentage form

[sens_vec_leaveout, spec_vec_leaveout, acc_vec_leaveout, weight_vec_leaveout] = SensSpecCalcFromConfMat(sum(conf_mat_leaveout,3), 1:label_num);
disp(mean([acc_vec_leaveout, sens_vec_leaveout, spec_vec_leaveout],1));
disp(weight_vec_leaveout'*[acc_vec_leaveout, sens_vec_leaveout, spec_vec_leaveout]); % weighted mean...

% plot and print classification results
if (PLOT_RESULTS)
    for slide_indx = 1:slide_num
        figure;
        % plot ground-truth slide
        subplot(1,2,1); imagesc(reshape(blk_label{slide_indx},Slide{slide_indx}.blk_num_y,Slide{slide_indx}.blk_num_x));
        ...
        % plot leaveout results
        subplot(1,2,2); imagesc(reshape(blk_mdl_leaveout{slide_indx},Slide{slide_indx}.blk_num_y,Slide{slide_indx}.blk_num_x));
        axis image; caxis([-1,label_num]); colormap(color_mat); % the most "diverse" slice
